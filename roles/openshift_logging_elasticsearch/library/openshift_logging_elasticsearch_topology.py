...
EXAMPLES = """
- action: opneshift_logging_topology
"""
...
class OpenshiftLoggingTopology():
    ...
    def find_similar_clientdata_node(self, nodes_ex, cur_node):
        candidates = []
        nd_storage = cur_node['node_storage_type']
            ...
            if nodes_ex[node].get('nodeSelector', {}) == cur_node.get('nodeSelector', {}) and\
               nodes_ex[node].get('node_storage_type') == nd_storage:
                if (nd_storage == 'emptydir') or (nd_storage == 'hostmount' and\
                    ...
                    return node
        raise Exception("Unable to reconcile Elasticsearch node topology. Existing node doesn't fit in the desired topology")
    ...
    def build_facts(self):
        ...
        self.add_facts_for("elasticsearch", "existing_topology", self._existing_topology)
        ...
        self.add_facts_for("elasticsearch", "node_topology", self._node_topology)
